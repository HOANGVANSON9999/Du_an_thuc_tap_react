...
const LichSu = (props) => {
  ...
  const getListPro = async () => {
    let url_api_lichsu = 'http://192.168.1.59:9999/hoadon/' + loginInfo._id
    try {
  ...
  const renderCartItem = ({ item }) => {
    return (
      <View style={styles.cartItemContainer}>
        ...
        <Text style={styles.productName}>Phương thức thanh toán:{item.pttt}</Text>
          <Text style={styles.productName}>Tên người mua:{item.tennguoimua}</Text>
          <Text style={styles.productPrice}>Price: ${item.tongtien}</Text>
        </View>
      </View>
    );
  }
